...
npx solcjs --bin --abi --optimize -o build/ contracts/PasswordVerifier.sol
...
contract_bin=$(cat build/contracts_PasswordVerifier_sol_Groth16Verifier.bin)
contract_abi=$(cat build/contracts_PasswordVerifier_sol_Groth16Verifier.abi)
...
echo "$contract_abi" > build/PasswordVerifierABI.json
echo "ABI saved to build/PasswordVerifierABI.json"
