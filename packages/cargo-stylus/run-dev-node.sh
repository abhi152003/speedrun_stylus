...
solcjs --bin --abi --optimize -o build/ contracts/BalanceChecker.sol
...
contract_bin=$(cat build/contracts_BalanceChecker_sol_Groth16Verifier.bin)
contract_abi=$(cat build/contracts_BalanceChecker_sol_Groth16Verifier.abi)
...
echo "$contract_abi" > build/BalanceCheckerABI.json
echo "ABI saved to build/BalanceCheckerABI.json"
